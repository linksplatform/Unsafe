...
namespace Platform.Unsafe
{
    ...
    public unsafe static class IntPtrExtensions
    {
        ...
        /// <summary>
        /// <para>Reads the element's <paramref name="value"/> from a position defined by the specified <paramref name="index"/> relative to the specified <paramref name="pointer"/>.</para>
        /// <para>Считывает <paramref name="value"/> элемента из позиции определяемой указанным <paramref name="index"/> относительно указанного <paramref name="pointer"/>.</para>
        /// </summary>
        ...
        /// <para>Порядковый номер элемента типа <typeparamref name="TValue"/>, который используется в качестве смещения относительно <paramref name="pointer"/>. Отсчёт элементов начинается с нуля.</para>
        /// </param>
        /// <returns><para>The element's value.</para><para>Значение элемента.</para></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
